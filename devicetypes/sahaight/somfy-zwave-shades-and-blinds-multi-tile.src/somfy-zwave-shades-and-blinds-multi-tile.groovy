 ...
 */
  metadata {
    definition (name: "Somfy Z-Wave Shades and Blinds Multi tile", namespace: "sahaight", author: "Eric, Ash, Stewart, Others") {
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd)
{
    ...
    log.debug "1 Basic report cmd.value:  ${cmd.value}"
...
def zwaveEvent(physicalgraph.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) {
    ...
    log.debug "2 SwitchBinaryReport cmd.value:  ${cmd.value}"
    log.debug settings?.shadeType
    log.debug settings?.openingTime
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv1.SwitchMultilevelReport cmd)
{
    ...
    log.trace "3 SwitchMultilevelReport cmd.value:  ${cmd.value}"
...
def on() {
    log.debug "on() treated as open()"
    setLevel(100) 
}

def off() {
    log.debug "off() treated as close()"
    setLevel(0) 
}

def setLevel() {
    log.debug "setLevel() treated as preset position"
    setLevel(50) 
}
...
def setLevel(level) {
    ...
    def newlevel = 0
    def openTime = settings?.openingTime.toInteger()
    def currstat = device.latestValue("windowShade")

	if ( currstat == "partially open" || level == 50 ) {
    	openTime = openTime / 2
    }
	int delayTime = openTime * 1000
	int runinTime = openTime
    ...
    if (level > null) {
        ...
        if (level >= 75) {
            ...
            runIn(runinTime, "finishOpenShade", [overwrite: true])
            delayBetween([
...
//                sendEvent(name: "switch", value: "on")
            ...
            runIn(runinTime, "finishCloseShade", [overwrite: true])
            if (settings.shadeType == "shades") {
                delayBetween([
...
//                    sendEvent(name: "switch", value: "off")
            ...
            sendEvent(name: "level", value: level)
            sendEvent(name: "switch", value: "on")
            runIn(runinTime, "finishPartialOpenShade", [overwrite: true])
            if (settings.shadeType == "shades") {
                delayBetween([
                    zwave.switchMultilevelV1.switchMultilevelStopLevelChange().format(),
                    zwave.basicV1.basicGet().format()
